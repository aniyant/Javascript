...
data.map(({ submission_link: url, id }) => {
  describe("Test", () => {
    ...
    it(`Check markup of individual todo item`, () => {
      cy.get(`#todo-wrapper input[type="checkbox"]`).should(
        "have.length",
        todosData.length
      );
      cy.get(`#todo-wrapper label`).should(
        "have.length",
        todosData.length
      );
      cy.then(() => {
    ...
    it(`Check if toggle of completed is persisted in DB`, () => {
      ...
      cy.get(`#todo-wrapper label input[type="checkbox"]`)
        .eq(0)
        .should("not.be.checked");
      cy.get(`#todo-wrapper label input[type="checkbox"]`)
        .eq(2)
        .should("be.checked");
      cy.get(`#todo-wrapper label input[type="checkbox"]`).eq(0).check();
      ...
      cy.get(`#todo-wrapper label input[type="checkbox"]`).eq(2).uncheck();
      cy.wait("@updateTodo").then(() => {
        cy.get(`#todo-wrapper label input[type="checkbox"]`)
          .eq(2)
          .should("not.be.checked");
